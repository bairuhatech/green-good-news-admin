...
function DataTable(props: any) {
	...
	const columns: ColumnsType<DataType> = [
		...
		},
		{
			...
			render: (text, record: any) => {
				return (
					<div style={{ fontSize: "10px" }}>{record.attributes.Title}</div>
				);
			},
		},
		{
		...
		},
		{
			...
			render: (text, record) => (
				<Space size="middle">
					<Tag color="lime" style={{ fontSize: "10px" }}>
						Published
					</Tag>
				</Space>
			),
	...
	const onDeleteItem = (id: any) => {
		let url = `https://green-good-news-server-9a460dab0f63.herokuapp.com/api/all-news/${id}`;
		axios
			.delete(url)
			.then((response) => {
